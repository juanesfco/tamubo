...
struct Options {
    std::string a_path;
    std::string b_path;
    std::string out_ab_path = "AB_cublas.bin";
    std::string out_ba_path = "BA_cublas.bin";
    int hold_ms = 0;
    int repeats = 1;
    int cycles = 1;
};
...
Options parse_args(int argc, char** argv) {
    ...
    for (int i = 1; i < argc; ++i) {
        ...
        if (std::strcmp(argv[i], "--a") == 0) {
            ...
            std::cout
                << "Usage: matrix_products_cublas_mpi --a A.bin --b B.bin "
                << "[--out-ab AB_cublas.bin] [--out-ba BA_cublas.bin] "
...
std::vector<double> run_product(
    ...
    if (static_cast<std::size_t>(n_int) != n) {
        throw std::runtime_error("matrix dimension is too large for cuBLAS int API");
    }

    std::vector<double> host_result(elements);

    std::cout << "rank " << rank << " starting cuBLAS " << label
              << " on pid=" << getpid()
    ...
    for (int cycle = 1; cycle <= options.cycles; ++cycle) {
        ...
        std::string cycle_label = "cuBLAS " + label;
        if (options.cycles > 1) {
            cycle_label += " cycle " + std::to_string(cycle) + "/" + std::to_string(options.cycles);
        }

        print_device_memory(rank, cycle_label + " before cublasCreate");
        cublasHandle_t handle = nullptr;
        CUBLAS_CHECK(cublasCreate(&handle));
        print_device_memory(rank, cycle_label + " after cublasCreate");
        ...
        for (int repeat = 0; repeat < options.repeats; ++repeat) {
            // Input/output files are row-major. cuBLAS uses column-major, so
            // row-major C = left * right is produced by column-major right * left.
        ...
        print_device_memory(rank, cycle_label + " after cublasDgemm");
        ...
        CUDA_CHECK(cudaDeviceSynchronize());
        print_device_memory(rank, cycle_label + " after cublasDestroy");
    ...
    std::cout << "rank " << rank << " finished all cuBLAS " << label
...
void compute_and_save(
    ...
    std::cout << "rank " << rank << " wrote cuBLAS " << label << " to " << output_path << "\n";
}
...
int main(int argc, char** argv) {
    ...
    try {
        ...
        if (rank == 0) {
            std::cout << "matrix_products_cublas_mpi: mpi_size=" << size
        ...
        std::cerr << "rank " << rank << " matrix_products_cublas_mpi: " << exc.what() << "\n";
        MPI_Abort(MPI_COMM_WORLD, EXIT_FAILURE);
    }
