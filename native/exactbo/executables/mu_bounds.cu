...
constexpr char kInputMagic[8] = {'T', 'M', 'U', 'B', 'I', 'N', '1', '!'};
constexpr char kOutputMagic[8] = {'T', 'M', 'U', 'B', 'O', 'U', '1', '!'};
...
MuInput read_input(const std::string& path) {
    std::ifstream in(path, std::ios::binary);
    if (!in) throw std::runtime_error("failed to open input file: " + path);
    char magic[8]{};
    in.read(magic, sizeof(magic));
    if (!in || std::memcmp(magic, kInputMagic, sizeof(magic)) != 0) throw std::runtime_error("invalid mu_bounds input magic");
    MuInput input;
    ...
    std::uint64_t elements = input.n * input.N;
    input.alpha.resize(input.N);
    input.K_lo.resize(elements);
    input.K_hi.resize(elements);
...
__global__ void mu_bounds_kernel(const double* alpha, const double* K_lo, const double* K_hi, std::uint64_t local_n, std::uint64_t N, double y_mean, double y_std, std::uint64_t scaled, double* mu_lo, double* mu_hi) {
    for (std::uint64_t row = static_cast<std::uint64_t>(blockIdx.x) * blockDim.x + threadIdx.x;
        ...
        for (std::uint64_t j = 0; j < N; ++j) {
            ...
            std::uint64_t idx = row * N + j;
            lo += K_lo[idx] * a_pos + K_hi[idx] * a_neg;
            hi += K_hi[idx] * a_pos + K_lo[idx] * a_neg;
        }
        if (!scaled) {
            lo = y_mean + y_std * lo;
            hi = y_mean + y_std * hi;
...
void compute_local(const MuInput& input, int rank, int size, int device, std::vector<double>& local_lo, std::vector<double>& local_hi) {
    ...
    std::cout << "rank " << rank << " starting mu_bounds pid=" << getpid() << " launch_cpu=" << current_cpu()
    ...
    std::size_t alpha_bytes = input.alpha.size() * sizeof(double);
    std::size_t K_bytes = local_K_lo.size() * sizeof(double);
    std::size_t out_bytes = local_n * sizeof(double);
    double *d_alpha=nullptr, *d_K_lo=nullptr, *d_K_hi=nullptr, *d_lo=nullptr, *d_hi=nullptr;
    print_device_memory(rank, "before cudaMalloc");
    CUDA_CHECK(cudaMalloc(&d_alpha, alpha_bytes));
    ...
    CUDA_CHECK(cudaMemcpy(d_alpha, input.alpha.data(), alpha_bytes, cudaMemcpyHostToDevice));
    ...
    mu_bounds_kernel<<<grid, block>>>(d_alpha, d_K_lo, d_K_hi, local_n, input.N, input.y_train_mean, input.y_train_std, input.scaled_output, d_lo, d_hi);
    ...
    CUDA_CHECK(cudaFree(d_hi)); CUDA_CHECK(cudaFree(d_lo)); CUDA_CHECK(cudaFree(d_K_hi)); CUDA_CHECK(cudaFree(d_K_lo)); CUDA_CHECK(cudaFree(d_alpha));
    CUDA_CHECK(cudaDeviceSynchronize());
...
int main(int argc, char** argv) {
    ...
    try {
        Options options = parse_args(argc, argv, "Usage: exactbo_mu_bounds --input input.bin --output output.bin");
        MuInput input = read_input(options.input_path);
        ...
        if (rank == 0) std::cout << "exactbo_mu_bounds: mpi_size=" << size << " device_count=" << device_count << " input=\"" << options.input_path << "\" output=\"" << options.output_path << "\" n=" << input.n << " N=" << input.N << "\n";
        ...
        if (rank == 0) { write_output(options.output_path, input.n, all_lo, all_hi); std::cout << "rank 0 wrote mu_bounds output to " << options.output_path << "\n"; }
    } catch (const std::exception& exc) {
        std::cerr << "rank " << rank << " exactbo_mu_bounds: " << exc.what() << "\n";
        MPI_Abort(MPI_COMM_WORLD, EXIT_FAILURE);
    }
    MPI_Finalize();
    return EXIT_SUCCESS;
