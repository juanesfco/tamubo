# -*- coding: utf-8 -*-
"""
Created on Thu Nov 12 13:51:02 2022

@author: richardcouperthwaite. Corrections by Danial Khatamsaz
"""

import numpy as np
from scipy.stats import norm
from numpy import random ## 012925 - Changed scipy by numpy
...
def expected_improvement(curr_max, y, std):
    ...
    pdf = norm.pdf((-curr_max+y)/std)
    cdf = norm.cdf((-curr_max+y)/std)

    EI = (-curr_max+y)*cdf + std*pdf
        
    max_val = np.max(EI)
    x_star = np.argmax(EI)
    
    return max_val, x_star, EI
